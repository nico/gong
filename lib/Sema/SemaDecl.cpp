...
#include "gong/Lex/HeaderSearch.h" // FIXME: Sema shouldn't depend on Lex
#include "gong/Lex/ModuleLoader.h" // FIXME: Sema shouldn't depend on Lex
#include "gong/Lex/Preprocessor.h" // FIXME: Sema shouldn't depend on Lex
#include "gong/Parse/ParseDiagnostic.h"
#include "gong/Sema/CXXFieldCollector.h"
...
Sema::ActOnAnonymousField(SourceLocation StarLoc, DeclArg TypeArg, Scope *S) {
  ...
  NameTypeDecl *NameType = TypeArg.takeAs<NameTypeDecl>();
  TypeDecl *FieldType = NameType;
  if (StarLoc.isValid())
    FieldType = ActOnPointerType(StarLoc, Owned(FieldType)).takeAs<TypeDecl>();

  FieldSpecDecl *FieldSpec =
      FieldSpecDecl::Create(Context, CurContext, FieldType->getLocation());

  CurContext->addDecl(FieldSpec);
  PushDeclContext(S, FieldSpec);
  AnonFieldDecl *New =
      AnonFieldDecl::Create(Context, FieldSpec, StarLoc, NameType);
  New->setType(Context.getTypeDeclType(FieldType));
  CheckRedefinitionAndPushOnScope(*this, FieldSpec, S, New);
  PopDeclContext();
...
Sema::ActOnFunctionDeclarator(Scope *S, Declarator &D, DeclContext *DC,
  ...
  // If there's a #pragma gong arc_cf_code_audited in scope, consider
