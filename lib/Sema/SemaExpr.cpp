...
#include "gong/Sema/AnalysisBasedWarnings.h"
...
static void CheckForNullPointerDereference(Sema &S, Expr *E) {
  ...
  // to get a deterministic trap and are surprised by gong's behavior.  This
...
Sema::BuildDeclRefExpr(Decl *D, SourceLocation L, const Type *Ty
                       //, ExprValueKind VK,
...
Sema::ActOnOperandName(SourceLocation IILoc, IdentifierInfo *II,
  ...
  // FIXME: consts, types, functions...
  if (VarDecl *VD = dyn_cast<VarDecl>(D)) {
    Ty = VD->getType();
    // FIXME: remove this once all exprs have a type.
    if (!Ty) Ty = Context.UnknownAnyTy;
  }
  return BuildDeclRefExpr(R.getFoundDecl(), IILoc, Ty);
}
