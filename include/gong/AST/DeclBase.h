...
template <> struct PointerLikeTypeTraits<gong::DeclContext *> {
...
// StoredDeclsMap* is only 4-byte aligned on 32-bit systems.
template <> struct PointerLikeTypeTraits<gong::StoredDeclsMap *> {
  typedef gong::StoredDeclsMap* PT;
public:
  static inline void *getAsVoidPointer(PT P) { return P; }
  static inline PT getFromVoidPointer(void *P) {
    return static_cast<PT>(P);
...
public:
  ...
  // FIXME: If implementing this, see PR12222
...
struct isa_impl<To, ::gong::DeclContext> {
  static bool doit(const ::gong::DeclContext &Val) {
...
struct cast_convert_val< ::gong::DeclContext, FromTy, FromTy> {
  static ::gong::DeclContext &doit(const FromTy &Val) {
...
template<class FromTy>
struct cast_convert_val< const ::gong::DeclContext, FromTy, FromTy> {
  static const ::gong::DeclContext &doit(const FromTy &Val) {
    return *FromTy::castToDeclContext(&Val);
  }
};
