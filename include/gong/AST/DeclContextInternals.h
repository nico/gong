...
//
//===----------------------------------------------------------------------===//
#ifndef LLVM_GONG_AST_DECLCONTEXTINTERNALS_H
#define LLVM_GONG_AST_DECLCONTEXTINTERNALS_H
...
public:
  ...
  void AddSubsequentDecl(NamedDecl *D) {
    ...
    //if (D->hasTagIdentifierNamespace())  // FIXME
      //Vec.push_back(D);

    // All other declarations go at the end of the list, but before any
    // tag declarations.  But we can be clever about tag declarations
    // because there can only ever be one in a scope.
    /*} else if (Vec.back()->hasTagIdentifierNamespace()) {  FIXME
...
class StoredDeclsMap
  : public llvm::SmallDenseMap<IdentifierInfo*, StoredDeclsList, 4> {
